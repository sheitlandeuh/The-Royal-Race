...
function openSettings(){$('#panelTitle').textContent='Réglages';$('#panel .card').classList.remove('wide');const lv=settings.get('q');
 ...
 <section><h4>SAUVEGARDE</h4><p class="hint">Ta partie est enregistrée dans ce navigateur (${stable.data.horses.length} chevaux, ${fmt(state.gold)} or).</p><button class="action" id="resetSave">Recommencer une partie</button></section></div>`;
