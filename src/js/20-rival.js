...
const rival=(()=>{const C=career.data;C.rival=C.rival||{w:0,l:0,lvl:0,met:false};const R=C.rival;
 ...
 function inject(F){const r=F.rivals[0],h=stable.active(),boost=1+Math.min(4,R.lvl)*1.5,st={};for(const s of STATS)st[s.k]=Math.min(99,r.stats[s.k]+boost);r.stats=st;r.name=VALMONT.name;r.livery={...VALMONT.livery};r.talent=VALMONT.talent;r.pref=RACE.dist;r.rating=Math.round(st.vit*.24+st.acc*.2+st.end*.18+st.dep*.1+st.tac*.15+st.tem*.13)-7;r.nemesis=true;return F}
