...
const replays=(()=>{const KEY='trr.replays',clone=o=>JSON.parse(JSON.stringify(o));let rec=null,busy=false,toCheck=null;
 ...
 hooks.on('race:start',()=>{if(busy)return;rec={v:1,at:new Date().toISOString(),race:clone(RACE),field:clone(currentField),seed:raceSeed,player:clone(racePlayer),strategy:state.strategy,horse:stable.active().name,inputs:[]}});
 ...
 // refait la course sans rien afficher ni jouer de son, puis restaure l'état
 function verify(r){if(busy||raceLoop>0||coach.open)return{ok:null,why:'course ou conseil en cours'};busy=true;
  const keep={RACE,currentField,racePlayer,strategy:state.strategy,complete:completeRace,say:sound.say,said:runRaceV2.said,tipd:runRaceV2.tipd,
  ...
  try{completeRace=()=>{};sound.say=()=>{};try{Object.defineProperty(navigator,'vibrate',{value:()=>false,configurable:true})}catch(e){}runRaceV2.said=1;runRaceV2.tipd=1;
   ...
   let i=0,n=0;const I=r.inputs;
   while(finishOrder.length<6&&n<9000){while(i<I.length&&I[i][0]<=raceTime){const[,type,val]=I[i++];if(type==='steer')steer(val);else if(type==='sprint')sprint();else if(type==='moment')moments.choose(val)}runRaceV2();n++}
   ...
   return{ok:same,order:finishOrder.slice(),expected:r.result.order,times,expectedTimes:r.result.times}}
  finally{raceLoop=null;({progress,visualProgress,raceFinished,raceFinishTimes,finishOrder,rivalAI,raceTime,playerEnergy,playerFinal,playerLane,autoSpeed,raceSeed,raceRng}=keep);keep.lanes.forEach((v,i)=>rivalLanes[i]=v);Object.assign(RS,keep.RS);RACE=keep.RACE;currentField=keep.currentField;racePlayer=keep.racePlayer;state.strategy=keep.strategy;completeRace=keep.complete;sound.say=keep.say;try{delete navigator.vibrate}catch(e){}runRaceV2.said=keep.said;runRaceV2.tipd=keep.tipd;busy=false}}
