...
const res=await p.evaluate(()=>{
 function sim(meet,seed,tactic,mode){RACE={...MEETINGS.find(m=>m.id===meet)};state.strategy=tactic;career.data.rival.lvl=0;const h=stable.active();h.fatigue=10;h.form=62;h.moral=72;currentField=null;buildField(seed);state.feed=99999;startRace();threeRace.startPhase='waiting';threeRace.goTime=performance.now();launchFromStalls();clearInterval(raceLoop);raceLoop=-1;autoSpeed=.5;
  ...
  const r={riv:finishOrder.indexOf(1)<finishOrder.indexOf(0),rank:finishOrder.indexOf(0)+1,stars:(()=>{try{return raceAnalysis(finishOrder.indexOf(0)+1).stars}catch(e){return e.message}})()};leaveRace();return r}
 const out={};for(const m of ['m1','m2','m3'])for(const mode of['ideal'])for(const t of(mode==='ideal'?['leader','stalker','finisher']:['stalker'])){let s=0,w=0,st=0;for(let k=1;k<=20;k++){const r=sim(m,k*7919,t,mode);s+=r.rank;w+=r.rank===1;st+=r.stars}out[`${m} ${mode} ${t}`]=`rang ${(s/20).toFixed(2)} · victoires ${w}/20 · étoiles ${(st/20).toFixed(1)}`}
 return out});
