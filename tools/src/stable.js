...
const stable=(()=>{
 ...
 if(!S||S.v!==1){S=fresh();try{const old=JSON.parse(localStorage.getItem('trr.champion')||'null');if(old){S.created=true;S.silks={main:old.main,second:old.second,pattern:old.pattern,cap:old.cap};S.horses[0].name=old.name;S.horses[0].coat=old.coat}}catch(e){}}
 ...
 function racePerf(st,cond={},dist=1600,pref=1600,tactic='stalker'){const c=1+((cond.form??60)-60)*.0009-Math.max(0,(cond.fatigue??0)-40)*.0016+((cond.moral??65)-65)*.0003,fit=1-Math.min(.05,Math.abs(dist-pref)/400*.018);
  const t={leader:{c:1.025,d:1.18,s:0},stalker:{c:1,d:1,s:0},finisher:{c:.975,d:.8,s:.03}}[tactic]||{c:1,d:1,s:0};
  return{cruise:(.418+st.vit*.0007)*c*fit,sprint:.1+st.acc*.0011+t.s,sprintDrain:.62*(1.4-st.acc*.006),drain:.105*(1.55-st.end*.0085),boxed:.27+st.tac*.0006,draft:.008+st.tac*.00012,noise:(100-st.tem)*.00008,window:180+(st.dep-50)*2.2,tactic:t}}
 ...
 return{get data(){return S},save,tick,active:()=>byId(S.active),byId,setActive(id){S.active=id;save()},train,care,spend,afterRace,preview,racePerf,rivals,rating,room,
